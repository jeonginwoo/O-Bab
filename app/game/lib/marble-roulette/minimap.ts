...
export class Minimap implements UIObject {
  ...
  onMouseMove(e?: { x: number; y: number }) {
    ...
    if (this._onViewportChangeHandler) {
      this._onViewportChangeHandler({
        x: this.mousePosition.x / this.currentScale,
        y: this.mousePosition.y / this.currentScale,
      });
    }
  }
  ...
  render(ctx: CanvasRenderingContext2D, params: RenderParameters) {
    ...
    // Calculate scale to fit canvas height with some padding
    const padding = 20;
    const availableHeight = params.size.y - (padding * 2);
    // Safety check for invalid size to prevent crash
    if (availableHeight <= 0 || stage.goalY <= 0) return;
