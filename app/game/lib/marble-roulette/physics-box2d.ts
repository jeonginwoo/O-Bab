...
export class Box2dPhysics implements IPhysics {
  private static loadedBox2D: typeof Box2D & EmscriptenModule;
  ...
  async init(): Promise<void> {
    console.log('Initializing Box2D...');
    try {
        if (Box2dPhysics.loadedBox2D) {
            this.Box2D = Box2dPhysics.loadedBox2D;
        } else {
            this.Box2D = await Box2DFactory({
            locateFile: (url: string) => {
                if (url.endsWith('.wasm')) {
                const wasmUrl = `${window.location.origin}/wasm/Box2D.wasm`;
                console.log('Locating Box2D WASM:', wasmUrl);
                return wasmUrl;
                }
                return url;
            }
            });
            Box2dPhysics.loadedBox2D = this.Box2D;
        }
        ...
        console.error('Failed to initialize Box2D:', e);
        throw e;
    }
  }
