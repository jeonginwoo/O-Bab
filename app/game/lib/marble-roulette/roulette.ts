...
export class Roulette extends EventTarget {
  ...
  constructor(container: HTMLElement) {
    ...
    this._renderer.init(container).then(() => {
      ...
      this._init().then(() => {
        ...
        if (this._isDestroyed) return;
        console.error('[Roulette] Initialization failed:', e);
        this.dispatchEvent(new CustomEvent('error', { detail: e }));
      });
    }).catch(e => {
        if (this._isDestroyed) return;
        console.error('[Roulette] Renderer initialization failed:', e);
    });
  }
  ...
  private _render() {
    if (!this._stage || !this.physics) return;
    try {
        const renderParams = {
          camera: this._camera,
          stage: this._stage,
        ...
        console.error('[Roulette] Render error:', e);
    }
  }
  ...
  public setMap(index: number) {
    ...
    this._loadMap();
    // Do not clear marbles if they exist, but we need to re-position them?
    // Actually, simply changing map makes old positions invalid.
    // The caller (React component) should handle re-adding marbles if needed,
    // or we should store names and re-add them here.
    
    // We will clear marbles here to ensure clean state for new map.
    // The React component will call setMarbles() immediately after to restore them.
